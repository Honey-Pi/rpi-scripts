    ...
    try:
        ...
        if modemisenabled:
                            ...
                            if os.path.exists('/dev/'+ surfstick['modem']):
                                ...
                                if surfstick['modem'] != modempath:
                                    if os.path.exists('/dev/'+ modempath):
                                        logger.info('Surfstick with ID ' + deviceid + ' ' + device['name'] + ' found in Modem mode on ' + device['device'] + ' with /dev/' + modempath + 'as interface')
                                        devicefound = True
                                        break
                                    else:
            ...
            if os.path.exists('/dev/'+ modempath):
                ...
                logger.error("Not starting wvdial as no modem on configurerd path /dev/" + str(modempath) + " found! Please check configuration or modem")
        else:
